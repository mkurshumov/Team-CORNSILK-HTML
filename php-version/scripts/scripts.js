...
function openJobDetail(number) {
	if(number == 1){
		var change = document.getElementById("jobDescription");
		 change.innerHTML = "Работа изисква поне 2 години опит като овчар, познаване на числата до 60 и служителят да притежава добре обучена кокошка по възможност петел, &#34;че бега по-бързо&#34;*. При преждевременно раждане пастирът да е запознт с методологията на израждане. Препоръчително е хората кандидастващи за работа да са преминали през цялостното обучение на ОвчУни *не покриваме разходите по отглеждане на пилето";
	}
	else if(number == 2) {
		var change = document.getElementById("jobDescription");
		 change.innerHTML = "Търсим хора готови да започнат революция в тази индустрия. Ще преработим кравите така че да дават уиски вместо мляко(Служителят се тества за алкохолизъм преди започване на работа), и лед вместо тор. Нашата идея е да ги вкараме във всяка чалготека за да може хората вечер да се чувстват полезни и да популяризираме изразът &#34;Снощи издои ли кравата ";
	}
	else if(number == 3) {
		var change = document.getElementById("jobDescription");
